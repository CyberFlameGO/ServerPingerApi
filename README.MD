...
All {ip} inputs accept either just the `ip` or `ip:port` format. If port cannot be parsed from the input an error will be returned.

All successful API calls return response 200, but when something goes wrong with pinging the server, an "error" field will be sent instead of server data.

## Minecraft

### Ping
Returns formatted information from as if the server was pinged from the game client.
```
https://pinggames.youhavetrouble.me/minecraft/v1/{ip}
```
#### Example response
```json
{
    "motd": "Hypixel Network  [1.8-1.17]\\n          SKYBLOCK, BEDWARS + MORE",
...
### Query
Returns formatted information from a query ping. This will return an error if the server does not have query enabled.
```
https://pinggames.youhavetrouble.me/minecraft/v1/query/{ip}
```
#### Example response
```json
{
  "server": {
    ...
    "plugins": [
      "LuckPerms 5.3.16",
      "CommandWhitelist 2.0",
      "Essentials 2.19.0-dev+188-d78729f"
    ],
    "players": [],
    "gametype": "SMP",
    "version": "1.17",
    "mod_name": "Purpur on 1.17-R0.1-SNAPSHOT",
    "game_id": "MINECRAFT"
  },
  "host": {
    "port": 25565,
...
### All information
Returns server information and player list from the server ping.
```
https://pinggames.youhavetrouble.me/source/v1/{ip}
```
#### Example response
```json
{
    "serverInfo": {
        "gameId": 3139621093375,
        "dedicated": 100,
        "networkVersion": 17,
        "maxPlayers": 64,
        "serverName": "[GFLClan.com] Zombie Escape 24/7 | Rank | Recruiting | NoBlock ",
        "secure": true,
        "serverPort": 27015,
        "gameDir": "csgo",
        "operatingSystem": 119,
        "serverId": 87240960559284224,
        "numberOfPlayers": 1,
        "serverTags": "_,!,bhopping,bhop,competitive,GFL,empty,escape,life,for,games,fun,gflclan,no lag,secure,unique,updated,ze,zombie,zombie escape,",
        "appId": 730,
        "numberOfBots": 0,
        "passwordProtected": false,
        "gameVersion": "1.37.9.4",
        "gameDescription": "Counter-Strike: Global Offensive",
        "mapName": "ze_shroomforest2_p6"
    },
    "players": [
        {
            "score": 201,
            "connectTime": 2760.6428,
            "name": "Noodlez"
        }
    ]
}
...
### Info
Returns the information from the server ping.
```
https://pinggames.youhavetrouble.me/source/v1/info/{ip}
```
#### Example response
```json
{
...
### Player list
Returns the player list from the server ping.
```
https://pinggames.youhavetrouble.me/source/v1/players/{ip}
```
#### Example response
```json
{
